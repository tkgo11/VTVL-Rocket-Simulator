...
export function HUD({ state }: HUDProps) {
  ...
  return (
    <div className="absolute top-4 left-4 flex flex-col gap-2 p-4 rounded bg-black/60 border border-slate-800 text-slate-200 font-mono text-sm shadow-xl backdrop-blur-sm pointer-events-none w-64">
      ...
      {isCrashed && (
        <div className="mt-4 p-2 bg-red-950/50 border border-red-900/50 rounded text-red-400 text-center text-xs animate-pulse">
          VEHICLE DESTROYED
        </div>
      )}
      {isLanded && (
        <div className="mt-4 p-2 bg-green-950/50 border border-green-900/50 rounded text-green-400 text-center text-xs">
          VEHICLE SECURE
        </div>
      )}
    </div>
